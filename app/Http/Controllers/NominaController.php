...
class NominaController extends Controller
{
    ...
    public function generarYDevolverRuta(Request $request)
    {
        $request->validate([
            'mes_anio' => 'required|date_format:Y-m',
        ]);
        ...
        // Ruta original donde están los PDFs
        $base = 'app/private/nominas/nominas_' . $anio . '/nomina_' . $mes . '_' . $anio;
        $empresaNorm = preg_replace('/[^A-Za-z0-9_-]/', '_', strtoupper($user->empresa->nombre ?? 'SIN_EMPRESA'));

        $carpetaUsuario = storage_path($base . '/' . $empresaNorm . '/' . $dniNormalizado);
        if (!is_dir($carpetaUsuario)) {
            $carpetaUsuario = storage_path($base . '/' . $dniNormalizado);
            if (!is_dir($carpetaUsuario)) {
                return response()->json(['error' => 'No se encontró la nómina.'], 404);
            }
        }

        $archivos = glob($carpetaUsuario . '/*.pdf');
        if (empty($archivos)) {
            return response()->json(['error' => 'No hay archivos PDF.'], 404);
        }
        ...
        foreach ($archivos as $archivo) {
            try {
                ...
                if (strpos($texto, $dniNormalizado) === false) continue;
        ...
        if (!$dniEnPdf) {
            return response()->json(['error' => 'DNI no encontrado en el PDF.'], 422);
        }

        // Guardar temporalmente
        $nombreArchivo = 'Nomina_' . $user->nombre_completo . '_' . $mes . '_' . $anio . '.pdf';
        $rutaPublica = 'tmp/nominas/' . uniqid() . '_' . $nombreArchivo;
        $rutaCompleta = storage_path('app/public/' . $rutaPublica);
        $pdf->Output($rutaCompleta, 'F');

        return response()->json([
            'url' => asset('storage/' . $rutaPublica),
