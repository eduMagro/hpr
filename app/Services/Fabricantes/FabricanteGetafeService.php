...
class FabricanteGetafeService implements FabricanteServiceInterface
{
    public function procesar(Request $request): void
    {
        ...
        try {
            $request->validate([
                ...
                'n_paquete.required' => 'El campo "nº paquete" es obligatorio.',
                'n_paquete.array' => 'El campo "nº paquete" debe ser un arreglo.',
                'n_paquete.*.string' => 'Cada valor de "nº paquete" debe ser una cadena de texto.',
            ...
            foreach ($request->producto_nombre as $index => $nombre) {
                ...
                $codigoGetafe = $request->producto_codigo[$index] ?? null;
                ...
                if (is_null($codigoGetafe) || is_null($tipoProducto) || is_null($diametro) || is_null($nPaquete) || is_null($pesoInicial) || is_null($ubicacionId)) {
                    throw new Exception("Datos incompletos para el producto en el índice {$index}");
                }

                // Validar formato del código de barras
                if (!preg_match('/^\d{9}$/', $codigoGetafe)) {
            ...
            \Log::error('Error al procesar el fabricante Getafe', [
                'message' => $e->getMessage(),
                'request' => $request->all(),
                'index' => $index ?? null,
            ]);

            throw new Exception('Error al procesar el fabricante Getafe: ' . $e->getMessage());
        }
    }
}
