...
class FabricanteSiderurgicaSevillanaService implements FabricanteServiceInterface
{
    public function procesar(Request $request): void
    {
        ...
        try {
            $request->validate([
                ...
                'cantidad_productos' => 'required|integer|min:1|max:25', // Cambiado de max:30 a max:25
                'fabricante' => 'nullable|string|max:255',
                'fabricante.*' => 'nullable|string|max:255',
                'producto_nombre' => 'required|array|min:1',
                'producto_nombre.*' => 'required|string|max:255',
                'producto_codigo' => 'required|array|min:1',
                'producto_codigo.*' => 'required|digits:20', // Cambiado de max:10 a max:20
                ...
                'cantidad_productos.max' => 'El campo "cantidad de productos" no puede ser mayor de 25.', // Actualizado
                ...
                'producto_codigo.*.digits' => 'Cada código de producto debe contener exactamente 20 dígitos.',
                'producto_codigo.*.required' => 'El campo "código del producto" es obligatorio.',
                'producto_codigo.*.max' => 'Cada código de producto no puede tener más de 20 caracteres.', // Actualizado
            ...
            foreach ($request->producto_nombre as $index => $nombre) {
                ...
                $codigoSS = $request->producto_codigo[$index] ?? null;
                ...
                // Extracción correcta de n_paquete
                $nPaquete = substr($request->producto_codigo[$index], 12, 7); // Extrae '8016155'

                // Validar datos obligatorios
                if (is_null($codigoSS) || is_null($tipoProducto) || is_null($diametro) || is_null($nPaquete) || is_null($pesoInicial) || is_null($ubicacionId)) {
                    throw new Exception("Datos incompletos para el producto en el índice {$index}");
                }

                // Validar formato del código de barras
                if (!preg_match('/^\d{20}$/', $codigoSS)) { // Actualizado a 20 dígitos
                    throw new Exception("El código de barras del fabricante no tiene el formato esperado en el índice {$index}. Debe ser un número de 20 cifras.");
                }

                // Crear el producto
                $producto = Producto::create([
                    'fabricante' => 'Siderúrgica Sevillana',
                    'nombre' => $nombre,
                    'tipo' => $tipoProducto,
                    'diametro' => $diametro,
                    'longitud' => $longitud,
            ...
            \Log::error('Error al procesar el fabricante Siderurgica Sevillana', [ // Corregido el nombre del fabricante
                'message' => $e->getMessage(),
                'request' => $request->all(),
                'index' => $index ?? null,
            ]);

            throw new Exception('Error al procesar el fabricante Siderurgica Sevillana: ' . $e->getMessage()); // Corregido el nombre del fabricante
        }
    }
}
