...
window.GhostPaquete = (function() {
     ...
     * @param {HTMLElement} config.grid - Elemento contenedor de la cuadrícula
     * @param {number} config.viewCols - Columnas de la vista
     * @param {number} config.viewRows - Filas de la vista
     * @param {number} config.W - Ancho real en celdas
     * @param {number} config.H - Alto real en celdas
     * @param {boolean} config.isVertical - Si está en modo vertical
     * @param {number} config.naveId - ID de la nave
     * @param {string} config.rutaGuardar - URL para guardar (POST)
     * @param {Function} config.getCeldaPx - Función para obtener tamaño de celda en px
     * @param {Function} config.onSuccess - Callback al guardar exitosamente
    ...
    function GhostPaquete(config) {
        ...
        this.confirmBeforeSave = config.confirmBeforeSave !== false; // default true
    ...
    GhostPaquete.prototype._ensureGhost = function() {
        ...
        this.ghost.className = 'ghost-paquete-mover'; // Usa clase CSS de styleLocIndex.css
        ...
        this.ghostActions.className = 'ghost-actions'; // Usa clase CSS de styleLocIndex.css
    ...
    GhostPaquete.prototype._enableDrag = function() {
        ...
        this._dragHandlers = { onDown: onDown, onMove: onMove, onUp: onUp };
    };
     ...
     * @param {string} paqueteData.codigo - Código del paquete
     * @param {number} paqueteData.paquete_id - ID del paquete
     * @param {number} paqueteData.longitud - Longitud en metros
     * @param {number} paqueteData.ancho - Ancho en metros
     */
    ...
    GhostPaquete.prototype.confirmar = async function() {
        ...
        if (x1r < 1 || y1r < 1 || x2r > this.W || y2r > this.H) {
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    ...
                    text: 'El paquete está fuera de los límites de la nave.',
                    toast: true,
                    position: 'top-end',
        ...
        if (btnConfirm) {
            ...
            btnConfirm.innerHTML = '<svg class="icon animate-spin" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none" opacity="0.25"/><path d="M12 2a10 10 0 0 1 10 10" stroke="currentColor" stroke-width="4" fill="none"/></svg>';
        }
        ...
        try {
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'No se pudo guardar la ubicación del paquete',
                    toast: true,
                    position: 'top-end',
                    showConfirmButton: false,
                    timer: 4000
