...
/* ===================== Sistema de selección múltiple de eventos ===================== */
window.eventosSeleccionados = window.eventosSeleccionados || [];

export function toggleSeleccionEvento(event, el) {
    const eventId = event.id;
    const index = window.eventosSeleccionados.findIndex(e => e.id === eventId);

    if (index > -1) {
        // Deseleccionar
        window.eventosSeleccionados.splice(index, 1);
        el.classList.remove('evento-seleccionado');
    } else {
        // Seleccionar solo si es tipo planilla
        const tipo = event.extendedProps?.tipo || 'planilla';
        if (tipo === 'planilla') {
            window.eventosSeleccionados.push({
                id: eventId,
                event: event,
                el: el
            });
...
export function limpiarSeleccion() {
    window.eventosSeleccionados.forEach(item => {
        if (item.el) item.el.classList.remove('evento-seleccionado');
    });
...
function actualizarContadorSeleccion() {
    let contador = document.getElementById('contador-seleccion');

    if (window.eventosSeleccionados.length > 0) {
        if (!contador) {
            contador = document.createElement('div');
            contador.id = 'contador-seleccion';
            contador.className = 'fixed bottom-4 right-4 z-50 bg-purple-600 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-3';
...
window.limpiarSeleccionEventos = limpiarSeleccion;
window.automatizarSalidasSeleccion = () => automatizarSalidas(null);

/* ===================== Automatizar Salidas ===================== */
export async function automatizarSalidas(calendar) {
    try { closeMenu(); } catch(_) {}

    if (window.eventosSeleccionados.length === 0) {
        return Swal.fire('⚠️', 'No hay agrupaciones seleccionadas.', 'warning');
    }

    // Construir las agrupaciones desde los eventos seleccionados
    const agrupaciones = window.eventosSeleccionados.map(item => {
        const ev = item.event;
        const p = ev.extendedProps || {};
        return {
            obra_id: p.obra_id || p.resourceId,
            fecha: ev.startStr?.split('T')[0] || ev.start?.toISOString().split('T')[0],
            planillas_ids: extraerPlanillasIds(ev),
            obra_nombre: p.obra || p.resourceTitle || 'Sin obra'
        };
    });

    // Confirmar la acción
    const confirmResult = await Swal.fire({
        title: '¿Automatizar salidas?',
        html: `
            <p class="mb-3">Se crearán salidas automáticas para ${agrupaciones.length} agrupación(es):</p>
            <div class="text-left max-h-48 overflow-y-auto text-sm">
                ${agrupaciones.map(a => `
                    <div class="p-2 mb-1 bg-gray-100 rounded">
                        <strong>${a.obra_nombre}</strong><br>
                        <span class="text-gray-600">Fecha: ${a.fecha} | ${a.planillas_ids.length} planilla(s)</span>
                    </div>
                `).join('')}
            </div>
    ...
    Swal.fire({
        title: 'Procesando...',
        html: 'Creando salidas automáticas...',
        allowOutsideClick: false,
        didOpen: () => Swal.showLoading()
    });

    try {
        const response = await fetch(window.AppSalidas?.routes?.automatizarSalidas, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': window.AppSalidas?.csrf || document.querySelector('meta[name="csrf-token"]')?.content,
                'Accept': 'application/json'
            },
            body: JSON.stringify({ agrupaciones })
        });

        const data = await response.json();

        if (data.requiere_decision && data.conflictos) {
            // Hay paquetes que ya están en otras salidas
            await manejarConflictosPaquetes(agrupaciones, data.conflictos, calendar);
            return;
        }
        ...
        if (data.success) {
            ...
            await Swal.fire({
                ...
                html: `
                    ...
                    <div class="mt-3 text-sm">
                        <p><strong>${data.total_salidas}</strong> salida(s) creada(s)</p>
                        <p><strong>${data.total_paquetes}</strong> paquete(s) asociado(s)</p>
                    </div>
                `,
                confirmButtonColor: '#16a34a'
            });
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: data.message || 'No se pudieron crear las salidas.'
            });
        }
    } catch (err) {
        console.error('Error en automatizarSalidas:', err);
        Swal.fire({
            icon: 'error',
...
async function manejarConflictosPaquetes(agrupaciones, conflictos, calendar) {
    const conflictosHtml = conflictos.map(c => `
        <tr>
            <td class="border px-2 py-1">${c.paquete_codigo}</td>
            <td class="border px-2 py-1">${c.planilla_codigo}</td>
            <td class="border px-2 py-1">${c.salida_actual}</td>
        </tr>
    `).join('');

    const result = await Swal.fire({
        title: 'Paquetes ya asignados',
        html: `
        ...
        showCancelButton: true,
        showDenyButton: true,
        confirmButtonText: 'Cambiar a nueva salida',
        denyButtonText: 'Crear sin estos paquetes',
        cancelButtonText: 'Cancelar',
    ...
    const accion = result.isConfirmed ? 'cambiar' : 'omitir';
    const paquetesConflictoIds = conflictos.map(c => c.paquete_id);

    // Reintentar con la acción seleccionada
    Swal.fire({
        title: 'Procesando...',
        html: 'Creando salidas automáticas...',
        allowOutsideClick: false,
    ...
    try {
        const response = await fetch(window.AppSalidas?.routes?.automatizarSalidas, {
            ...
            body: JSON.stringify({
                agrupaciones,
                accion_conflicto: accion,
                paquetes_conflicto_ids: paquetesConflictoIds
            })
        });
...
export function attachEventoContextMenu(info, calendar) {
    ...
    `;
        ...
        if (tipo === "planilla") {
            ...
            const estaSeleccionado = window.eventosSeleccionados?.some(e => e.id === ev.id);

            items = [
                {
                    label: estaSeleccionado ? "Deseleccionar" : "Seleccionar para automatizar",
                    icon: estaSeleccionado ? "☐" : "☑️",
                    onClick: () => {
                        toggleSeleccionEvento(ev, info.el);
                        closeMenu();
                    },
                },
                {
                    label: "Automatizar salidas",
                    icon: "🚀",
                    onClick: () => {
