                ...
                <div class="mb-6 p-4 bg-gray-50 rounded-lg flex flex-wrap items-center gap-3">
                    ...
                    <label class="text-sm font-semibold">
                        ...
                        <input type="date" id="fechaFin"
                               value="{{ $filtro_fecha_fin ?? now()->format('Y-m-d') }}"
                               class="border rounded text-sm px-2 py-1 ml-1">
                    </label>
                    <label class="text-sm font-semibold">
                        Turno:
                        ...
                        <div class="bg-white shadow rounded-lg p-3">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <select data-id="{{ $maquina->id }}" data-nombre="{{ $maquina->nombre }}"
                                    ...
                                    <option value="averiada" {{ $maquina->estado === 'averiada' ? 'selected' : '' }}>🔴 Averiada</option>
                                    <option value="mantenimiento" {{ $maquina->estado === 'mantenimiento' ? 'selected' : '' }}>🛠️ Mantenimiento</option>
            ...
            select.addEventListener('change', async function() {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        alert(`${colores[nuevoEstado]} Máquina "${this.dataset.nombre}" cambiada a: ${nuevoEstado}`);
                    } else {
