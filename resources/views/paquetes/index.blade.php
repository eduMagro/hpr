<x-app-layout>
    <x-slot name="header">
        <h2 class="text-lg font-semibold text-gray-800">
            <a href="{{ route('paquetes.index') }}" class="text-gray-600">
                {{ __('Paquetes') }}
            </a>
                    ...
                    @forelse ($paquetes as $paquete)
                        <tr class="border-b hover:bg-gray-100">
                            ...
                            <td class="px-4 py-2 flex space-x-2">
                                ...
                                <form action="{{ route('paquetes.destroy', $paquete->id) }}" method="POST" class="inline" onsubmit="return confirm('¿Seguro que deseas eliminar este paquete?')">
                                    @csrf
                                    @method('DELETE')
