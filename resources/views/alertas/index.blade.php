<x-app-layout>
        ...
        <div x-data="{ mostrarModal: false }">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 text-sm">
                        @forelse ($alertas as $alerta)
                            ...
                            <tr class="cursor-pointer transition-all duration-200 hover:bg-blue-50 hover:shadow-md {{ $noLeida ? 'bg-yellow-50 border-l-4 border-yellow-400' : 'bg-white' }}"
                                ...
                                <td class="px-4 py-3 text-center" onclick="event.stopPropagation()">
                                    ...
                                    @if ($noLeida)
                                        <div class="mt-1">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-yellow-400 text-yellow-900 animate-pulse">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex flex-col items-center">
                                        <span class="text-gray-900 font-medium">{{ $alerta->created_at->diffForHumans() }}</span>
                                        <span class="text-xs text-gray-500 mt-0.5">{{ $alerta->created_at->format('d/m/Y H:i') }}</span>
                                    </div>
                                </td>
            ...
            <div class="md:hidden space-y-3">
                @forelse ($alertas as $alerta)
                    ...
                    <div class="bg-white rounded-lg shadow-md overflow-hidden {{ $noLeida ? 'ring-2 ring-yellow-400 bg-yellow-50' : '' }}"
                        data-alerta-id="{{ $alerta->id }}">

                        <!-- Header de la card -->
                        <div class="bg-gradient-to-r {{ $alerta->tipo === 'entrante' ? 'from-green-500 to-green-400' : 'from-blue-500 to-blue-400' }} px-4 py-3 flex items-center justify-between">
                            <div class="flex items-center space-x-2">
                                ...
                                @if ($noLeida)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-yellow-400 text-yellow-900 animate-pulse">
                                        NUEVO
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="p-4 space-y-3" onclick="marcarAlertaLeida({{ $alerta->id }}, this.closest('[data-alerta-id]'), @js($alerta->mensaje_completo), {{ $esSaliente ? 'true' : 'false' }})">
    ...
    @if ($esAdministrador)
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-6 md:mt-8 mb-4 gap-3">
            ...
            <span class="text-xs sm:text-sm text-gray-600 bg-gray-100 px-3 py-1.5 rounded-full font-medium whitespace-nowrap">
        ...
        <div class="md:hidden bg-white rounded-lg shadow-md p-4 mb-4">
            <form method="GET" action="{{ route('alertas.index') }}" class="space-y-3">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg font-medium text-sm">
                        Filtrar
                    </button>
                    <a href="{{ route('alertas.index') }}" class="flex-1 bg-gray-500 hover:bg-gray-600 text-white py-2 px-4 rounded-lg font-medium text-sm text-center">
                        Limpiar
                    </a>
                </div>
            </form>
            ...
            <table class="w-full min-w-[1000px]">
                ...
                <tbody class="text-gray-700 text-sm">
                    @forelse ($todasLasAlertas as $alerta)
                        <tr class="border-b border-gray-200 odd:bg-gray-50 even:bg-white hover:bg-blue-50 transition-colors duration-150">
                            <td class="p-3 text-center border-r border-gray-200">
                                <div class="flex flex-col items-center">
                                    ...
                                    @if ($alerta->usuario1?->rol)
                                        <span class="text-xs text-gray-500 mt-0.5">{{ ucfirst($alerta->usuario1->rol) }}</span>
                                    @endif
                                </div>
                            </td>
                            <td class="p-3 text-center border-r border-gray-200">
                            ...
                            <td class="p-3 border-r border-gray-200">
                                <div class="max-w-md">
                                    <p class="text-gray-800 line-clamp-2 leading-relaxed" title="{{ $alerta->mensaje }}">
                                        {{ $alerta->mensaje }}
                                    </p>
        ...
        <div class="md:hidden space-y-4">
            @forelse ($todasLasAlertas as $alerta)
                <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 transform transition-all duration-200 hover:shadow-xl">

                    <!-- Header de la card con degradado -->
                    <div class="bg-gradient-to-r {{ $alerta->tipo === 'entrante' ? 'from-green-500 to-emerald-600' : 'from-blue-500 to-indigo-600' }} px-4 py-3">
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div class="flex items-start space-x-3 pb-3 border-b border-gray-100">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-400 mt-1">
                                    {{ $alerta->created_at?->format('H:i') }} · {{ $alerta->created_at?->diffForHumans() }}
                                </p>
                            </div>
                        </div>
                        ...
                        <!-- Mensaje -->
                        <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg p-4 border border-gray-200 shadow-inner">
    ...
    <div id="modalVerMensaje"
        ...
        <div class="bg-white rounded-t-2xl md:rounded-xl shadow-2xl w-full md:max-w-2xl max-h-[90vh] md:max-h-[85vh] overflow-y-auto transform transition-all duration-300"
            ...
            <!-- Header del modal -->
            <div class="bg-gradient-to-r from-blue-600 to-blue-500 px-4 md:px-6 py-4 md:rounded-t-xl flex items-center justify-between sticky top-0 z-10">
            ...
            <div class="p-4 md:p-6">
                ...
                <div id="contenidoMensaje" class="bg-gray-50 rounded-lg p-3 md:p-4 min-h-[120px] text-sm md:text-base text-gray-800 whitespace-pre-wrap border border-gray-200"></div>
                ...
                <!-- Botones de acción -->
                <div class="flex flex-col sm:flex-row justify-end gap-2 md:gap-3 mt-4 md:mt-6 hidden" id="botonesEdicion">
                ...
                <div id="botonesRespuesta" class="hidden flex flex-col sm:flex-row justify-end gap-2 md:gap-3 mt-4 md:mt-6">
    ...
    <div id="modalConfirmacionCambio"
        ...
        <div class="bg-white rounded-t-2xl md:rounded-xl shadow-2xl w-full md:max-w-md transform transition-all duration-300"
            ...
            <!-- Header del modal -->
            <div class="bg-gradient-to-r from-orange-500 to-orange-400 px-4 md:px-6 py-4 md:rounded-t-xl sticky top-0 z-10">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-2 md:space-x-3">
            ...
            <div class="p-4 md:p-6">
                <div class="bg-blue-50 border-l-4 border-blue-500 p-3 md:p-4 rounded-r-lg mb-4">
                    <p class="text-sm md:text-base text-gray-800 mb-3">
                        Solicitud para elemento: <strong class="text-blue-700 text-base md:text-lg"><span id="elementoModal"></span></strong>
                    </p>
                    <div class="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-4 text-sm">
                        ...
                        <svg class="w-5 h-5 sm:w-6 sm:h-6 text-gray-400 rotate-90 sm:rotate-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"/>
                        </svg>
                        <div class="flex items-center bg-green-100 px-3 py-2 rounded-lg w-full sm:w-auto justify-center">
    ...
    <script>
        ...
        function marcarAlertaLeida(alertaId, elemento, mensaje, esSaliente) {
            ...
            const noLeida = elemento.classList.contains('bg-yellow-50') || elemento.querySelector('.animate-pulse');

            if (noLeida && !esSaliente) {
                ...
                fetch("{{ route('alertas.verMarcarLeidas') }}", {
                    ...
                    .then(data => {
                        ...
                        // Recargar para actualizar el badge de "Nuevo"
            ...
            // Cargar el hilo de conversación
