<x-app-layout>
    ...
    <div class="w-full p-4 sm:p-6">
        ...
        {{-- Tabla de Salidas (agrupadas por mes) --}}
                                ...
                                @foreach ($salida->planillasUnicas as $planilla)
                                    <tr class="hover:bg-gray-50 text-sm">
                                        ...
                                        <td class="py-2 px-4 border-b editable" contenteditable="true"
                                            ...
                                            {{ $salida->importe_grua ?? '0' }}
                                        </td>
                                        <td class="py-2 px-4 border-b editable" contenteditable="true"
                                            data-id="{{ $salida->id }}" data-field="horas_almacen">
                                            {{ $salida->horas_almacen ?? '0' }}
                                        </td>
                                        <!-- Importe asignado: si está definido en la planilla, se muestra ese importe; si no, se muestra el importe de la salida -->
                                        <td class="py-2 px-4 border-b editable" contenteditable="true"
                                            data-id="{{ $salida->id }}" data-field="importe">
                                            {{ isset($planilla->importe_asignado) ? number_format($planilla->importe_asignado, 2) : number_format($salida->importe, 2) }}
                                        </td>
                                        <td class="py-2 px-4 border-b editable" contenteditable="true"
        ...
        {{-- Sección para la tabla de relaciones Salida - Cliente --}}
        <div class="mt-10">
            <h3 class="text-xl font-semibold text-gray-800 mb-4">Relaciones Salida - Cliente</h3>
            <div class="bg-white shadow-lg rounded-lg p-4 sm:p-6 overflow-x-auto">
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-gray-100 text-left text-sm font-medium text-gray-700">
                            <th class="py-2 px-4 border-b">Código Salida</th>
                            <th class="py-2 px-4 border-b">Cliente</th>
                            <th class="py-2 px-4 border-b">Horas Paralización</th>
                            <th class="py-2 px-4 border-b">Importe Paralización</th>
                            <th class="py-2 px-4 border-b">Horas Grua</th>
                    ...
                    <tbody>
                        @foreach ($salidaClientes as $relacion)
                            <tr class="hover:bg-gray-50 text-sm">
                                <td class="py-2 px-4 border-b">{{ $relacion->salida->codigo_salida }}</td>
                                <td class="py-2 px-4 border-b">{{ $relacion->cliente->nombre }}</td>
                                <td class="py-2 px-4 border-b">{{ $relacion->horas_paralizacion }}</td>
                                <td class="py-2 px-4 border-b">{{ number_format($relacion->importe_paralizacion, 2) }}
                                    €</td>
                                <td class="py-2 px-4 border-b">{{ $relacion->horas_grua }}</td>
                                <td class="py-2 px-4 border-b">{{ number_format($relacion->importe_grua, 2) }} €</td>
                                <td class="py-2 px-4 border-b">{{ $relacion->horas_almacen }}</td>
                                <td class="py-2 px-4 border-b">{{ number_format($relacion->importe, 2) }} €</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
