<section class="space-y-6">
    <header>
        <h2 class="text-lg font-medium text-gray-900">
            {{ __('Eliminar cuenta') }} wire:navigate
        </h2>

        <p class="mt-1 text-sm text-gray-600">
            {{ __('Una vez que tu cuenta sea eliminada, todos sus recursos y datos serán eliminados permanentemente. Antes de eliminar tu cuenta, por favor descarga cualquier dato o información que desees conservar.') }} wire:navigate
        </p>
    </header>
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy', $user->id) }}" class="p-6">
            ...
            <h2 class="text-lg font-medium text-gray-900">
                {{ __('¿Estás seguro de querer borrar tu cuenta?') }} wire:navigate
            </h2>

            <p class="mt-1 text-sm text-gray-600">
                {{ __('Una vez que tu cuenta sea eliminada, todos sus recursos y datos serán eliminados permanentemente. Por favor, ingresa tu contraseña para confirmar que deseas eliminar tu cuenta de forma permanente.') }} wire:navigate
            </p>
            ...
            <div class="mt-6">
                ...
                <x-input-error :messages="$errors->userDeletion->get('password')" wire:navigate class="mt-2 text-red-500" />
            </div>
