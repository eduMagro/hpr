<x-app-layout>
    ...
    <div class="w-full px-6 py-4">
        ...
        <div class="bg-white rounded-lg shadow max-w-4xl mx-auto px-6 py-4 space-y-6">
            <form method="POST" action="{{ route('pedidos-almacen-venta.store') }}" x-data="{ productos: [{}] }"
                class="space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <!-- Input texto (crear nuevo o buscar) -->
                    <div>
                        <label class="text-sm text-gray-600">Cliente (nuevo)</label>
                        <input type="text" name="cliente_nombre"
                            class="w-full border rounded px-3 py-2 text-sm focus:ring focus:border-blue-300"
                            placeholder="Escribe el nombre del cliente si no está en la lista">
                    </div>

                    <!-- Select de clientes existentes -->
                    <div>
                        <label class="text-sm text-gray-600">Cliente (existente)</label>
    ...
    <script>
        const URL_CLIENTES_BUSCAR = @json(route('clientes-almacen.verBuscar'));
        ...
        function clienteAutocomplete() {
            ...
            return {
                ...
                async buscar() {
                    ...
                    try {
                        const res = await fetch(`${URL_CLIENTES_BUSCAR}?query=${encodeURIComponent(query)}`, {
                            headers: {
                        ...
                        // 🔹 filtrar coincidencias en el cliente (extra, por si el backend devuelve más)
                        this.sugerencias = (Array.isArray(data) ? data : []).filter(c =>
                ...
                seleccionar(cliente) {
                    this.input = cliente.nombre;
                    this.cliente_id = cliente.id;
                    this.abierto = false;
                },

                cerrarConRetraso() {
                    setTimeout(() => (this.abierto = false), 200);
                },
            };
        }
    </script>
