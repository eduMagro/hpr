...
<script>
    (function() {
        if (window.innerWidth >= 768) {
            ...
            document.documentElement.classList.add(sidebarOpen ? 'sidebar-initially-open' :
...
@if (auth()->check() && auth()->user()->esOficina())
    <div x-data="{
        ...
        menuSectionIds: [@foreach($menuItems as $section)
            '{{ $section['id'] }}'
            @if(!$loop->last),
            @endif
            @endforeach
        ...
        menuItemCounts: {
            ...
            '{{ $section['id'] }}': {{ isset($section['submenu']) ? count($section['submenu']) : 0 }} @if(!$loop->last),
            @endif
            @endforeach
        },
        ...
        <div id="main-sidebar"
            ...
            <div class="p-3 space-y-2 border-b border-gray-800">
                ...
                <div class="relative">
                    <button @click="searchOpen = true"
                        ...
                        <span x-show="open" class="sidebar-text text-gray-400 group-hover:text-white transition">Buscar
                            (Ctrl+K)</span>
                    </button>
                </div>
                    ...
                    <div x-show="showRecent" x-cloak x-transition class="mt-2 space-y-1"
                        ...
                        <template x-for="page in recentPages.slice(0, 5)" :key="page.route">
                            <a :href="page.url || `{{ url('/') }}${getRouteUrl(page.route)}`" wire:navigate
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="text-gray-300 group-hover:text-white truncate" x-text="page.label">
                                    </div>
                        ...
                        <div class="relative" id="menu-section-{{ $section['id'] }}">
                            ...
                            if (open) {
                                ...
                                :class="{
                                    ...
                                    'ring-2 ring-blue-400 ring-offset-1 ring-offset-gray-900': focusedSectionId === '{{ $section['id'] }}' &&
                                    ...
                                    @else
                                        ...
                                        <div class="flex items-center group"
                                            ...
                                            <a href="{{ route($item['route']) }}" wire:navigate
                                                ...
                                                :class="{
                                                    'bg-{{ $section['color'] }}-500 text-white font-medium': currentPath &&
                                                        isRouteActive('{{ route($item['route']) }}'),
                                                    'text-gray-400 hover:text-white hover:bg-gray-800': currentPath && !
                                                        isRouteActive('{{ route($item['route']) }}'),
                                                    'ring-2 ring-blue-400 ring-offset-1 ring-offset-gray-900': focusedSectionId === '{{ $section['id'] }}' &&
                                                        focusedItemIndex === {{ $itemIndex }} && !isRouteActive(
                                                            '{{ route($item['route']) }}')
                                                }"
                                            ...
                                            <button
                                                ...
                                                <svg class="w-4 h-4 transition"
                                                    :class="isFavorite('{{ $item['route'] }}') ?
                                                        'text-yellow-500 fill-current' : 'text-gray-500'"
                                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
            ...
            <div class="p-3 border-t border-gray-800 space-y-2">
                ...
                <a href="{{ route('dashboard') }}" wire:navigate
                    ...
                    <span x-show="open" class="sidebar-text text-gray-400 group-hover:text-white">Dashboard</span>
                </a>

                <!-- Modo Oscuro -->
                <button @click="toggleDarkMode()"
                    class="w-full flex items-center space-x-3 px-3 py-2 rounded-lg hover:bg-gray-800 transition text-sm group">
                    <svg x-show="!darkMode" x-cloak class="w-5 h-5 flex-shrink-0 text-yellow-500" fill="currentColor"
                        viewBox="0 0 24 24">
                        <path
                            d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z">
                        </path>
                    </svg>
                    <svg x-show="darkMode" x-cloak class="w-5 h-5 flex-shrink-0 text-blue-400" fill="currentColor"
                        viewBox="0 0 24 24">
                        <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z">
                        </path>
                    </svg>
                    <span x-show="open" class="sidebar-text text-gray-400 group-hover:text-white"
        ...
        <div x-show="searchOpen" x-transition:enter="transition ease-out duration-200"
            ...
            x-transition:leave-end="opacity-0" @click.self="searchOpen = false" x-cloak
            ...
            <div @click.away="searchOpen = false"
                ...
                <div class="max-h-96 overflow-y-auto">
                    ...
                    <template x-for="(result, index) in searchResults" :key="result.route">
                        <a :href="result.url" class="block px-4 py-3 transition"
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="flex-1">
                                    <div class="font-medium text-gray-900 dark:text-white" x-text="result.label">
                                    </div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400" x-text="result.section">
                                    </div>
                                </div>
